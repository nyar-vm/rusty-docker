...
use std::{collections::HashMap, default::Default, fs, path::Path, time::SystemTime};

use docker_types::{
    ConfigInfo, DockerConfig, DockerError, EndpointConfig, EndpointInfo, EndpointStatus, EndpointType, LogConfig,
    ResourceLimits, SecretInfo,
};
...
impl ConfigManager {
    ...
    pub fn new() -> Result<Self> {
        ...
        // 确保配置、密钥和端点目录存在
        fs::create_dir_all(&configs_dir).map_err(|e| DockerError::io_error("create_dir_all", e.to_string()))?;
        fs::create_dir_all(&secrets_dir).map_err(|e| DockerError::io_error("create_dir_all", e.to_string()))?;
        fs::create_dir_all(&endpoints_dir).map_err(|e| DockerError::io_error("create_dir_all", e.to_string()))?;

        Ok(Self { config_path, configs_dir, secrets_dir, endpoints_dir })
    }
    ...
    fn create_default_config(path: &str) -> Result<()> {
        ...
        if let Some(dir) = Path::new(path).parent() {
            fs::create_dir_all(dir).map_err(|e| DockerError::io_error("create_dir_all", e.to_string()))?;
        }
    ...
    fn get_default_config() -> DockerConfig {
        DockerConfig {
            ...
            default_resources: ResourceLimits { cpu_limit: 1.0, memory_limit: 512, storage_limit: 10, network_limit: 10 },
    ...
    pub fn get_config(&self) -> Result<DockerConfig> {
        // 读取配置文件
        let config_content =
            fs::read_to_string(&self.config_path).map_err(|e| DockerError::io_error("read_to_string", e.to_string()))?;
    ...
    fn generate_id(data: &str) -> String {
        ...
        hasher.update(data);
        hasher.update(SystemTime::now().duration_since(SystemTime::UNIX_EPOCH).unwrap().as_secs().to_string());
    ...
    pub fn delete_config(&self, config_id: &str) -> Result<()> {
        ...
        fs::remove_file(&config_path).map_err(|e| DockerError::io_error("remove_file", e.to_string()))?;
        Ok(())
    }
    ...
    pub fn list_configs(&self) -> Result<Vec<ConfigInfo>> {
        ...
        // 遍历配置目录
        for entry in fs::read_dir(&self.configs_dir).map_err(|e| DockerError::io_error("read_dir", e.to_string()))? {
    ...
    pub fn create_secret(&self, name: &str, data: &str, labels: HashMap<String, String>) -> Result<SecretInfo> {
        ...
        let secret_info = SecretInfo { id: id.clone(), name: name.to_string(), created_at: SystemTime::now(), labels, digest };
    ...
    pub fn delete_secret(&self, secret_id: &str) -> Result<()> {
        ...
        // 删除密钥文件和数据文件
        fs::remove_file(&secret_path).map_err(|e| DockerError::io_error("remove_file", e.to_string()))?;
        if Path::new(&data_path).exists() {
            fs::remove_file(&data_path).map_err(|e| DockerError::io_error("remove_file", e.to_string()))?;
        }
    ...
    pub fn list_secrets(&self) -> Result<Vec<SecretInfo>> {
        ...
        // 遍历密钥目录
        for entry in fs::read_dir(&self.secrets_dir).map_err(|e| DockerError::io_error("read_dir", e.to_string()))? {
            let entry = entry.map_err(|e| DockerError::io_error("entry", e.to_string()))?;
            let path = entry.path();
    ...
    pub fn delete_endpoint(&self, endpoint_id: &str) -> Result<()> {
        ...
        fs::remove_file(&endpoint_path).map_err(|e| DockerError::io_error("remove_file", e.to_string()))?;
        Ok(())
    }
    ...
    pub fn list_endpoints(&self) -> Result<Vec<EndpointInfo>> {
        ...
        // 遍历端点目录
        for entry in fs::read_dir(&self.endpoints_dir).map_err(|e| DockerError::io_error("read_dir", e.to_string()))? {
            let entry = entry.map_err(|e| DockerError::io_error("entry", e.to_string()))?;
            let path = entry.path();
