use super::{BridgeNetworkDriver, NetworkConfig, NetworkDriver, NetworkInfo, NetworkManager, Result};
...
impl LinuxNetworkManager {
    pub fn new() -> Self {
        let manager = Self { networks: Arc::new(Mutex::new(HashMap::new())), drivers: Arc::new(Mutex::new(HashMap::new())) };
...
impl NetworkManager for LinuxNetworkManager {
    async fn create_network(&self, config: &NetworkConfig) -> Result<NetworkInfo> {
        ...
        let driver = {
            ...
            drivers_guard
                .get(&driver_name)
                .ok_or_else(|| DockerError::not_found("network driver", driver_name.clone()))?
                .clone()
        };
    ...
    async fn connect_container(&self, network_id: &str, container_id: &str, aliases: Option<Vec<String>>) -> Result<()> {
        ...
        let driver_name = {
            let mut networks_guard = networks.lock().await;
            let network =
                networks_guard.get(&network_id).ok_or_else(|| DockerError::not_found("network", network_id.clone()))?;
        ...
        let driver = {
            let mut drivers_guard = drivers.lock().await;
            drivers_guard.get(&driver_name).ok_or_else(|| DockerError::not_found("network driver", driver_name))?.clone()
        };
    ...
    async fn add_port_mapping(&self, container_id: &str, container_ip: &str, port_mapping: &super::PortMapping) -> Result<()> {
        // 默认使用 bridge 驱动
        ...
        let driver = {
            ...
            drivers_guard
                .get(driver_name)
                .ok_or_else(|| DockerError::not_found("network driver", driver_name.to_string()))?
                .clone()
        };
    ...
    async fn remove_port_mapping(&self, port_mapping: &super::PortMapping) -> Result<()> {
        // 默认使用 bridge 驱动
        let driver_name = "bridge";
        let drivers = self.drivers.clone();
